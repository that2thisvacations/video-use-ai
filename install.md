...
description: Install video-use into the current agent (Claude Code, Codex, Hermes, Openclaw, etc.) and wire up ffmpeg plus the local transcription stack so the user can start editing immediately.
---
...
3. Python 3.11 plus the local helper dependencies.
...
`pyproject.toml` lists `librosa`, `matplotlib`, `pillow`, `numpy`. No console scripts — helpers are invoked directly as `python helpers/<name>.py`.
...
If `brew` requires a sudo prompt, tell the user the exact command and wait. Do not invent a password.
...
### 5. Local transcription setup

Transcription is local-first. The default `placeholder` provider always works. Piper is optional and only needed if the user wants real local narration.

1. Check that the local helper runtime works:

    ```bash
    python3.11 -m pip show piper-tts >/dev/null 2>&1 || echo "piper optional"
    ```

2. Keep the placeholder path available for quick smoke tests:

    ```bash
    python helpers/transcribe.py --help >/dev/null
    ```

3. If the user asks for Piper, install it separately in the same Python environment and download a voice model into a local data directory.
...
Full transcription test is optional at install time — it is safe to wait until the user hands you their first clip.
